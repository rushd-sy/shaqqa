...
* **Acceptance Criteria:**
    * `GET /api/v1/advertisements/{id_advertisement}` returns full details (property specs, media, description, contact info).
...
* **Endpoint:** `PUT /api/v1/advertisements/{id_advertisement}`
...
* **Endpoint:** `DELETE /api/v1/advertisements/{id_advertisement}`
  ...
  "data": [
    {
      ...
      "media": [
      { "id_media": "1a2b3c4d-...-uuid", "is_cover": true, "display_order": 0 } // return the cover image only currently in this version
      ]
    }
  ],
  "pagination": {
...
* **Endpoint:** `GET /api/v1/advertisements/{id_advertisement}`
...
## 6. Add Media (Images)
* **Endpoint:** `POST /api/v1/advertisements/{id_advertisement}/media`
...
* **Endpoint:** `PATCH /api/v1/advertisements/{id_advertisement}/media/{id_media}`
...
* **Endpoint:** `DELETE /api/v1/advertisements/{id_advertisement}/media/{id_media}`
...
* **`id_advertisement`** (PK, UUID): Unique identifier for the advertisement **version**.
* **`id_user`** (FK -> `User.id_user`, UUID): Unique identifier of the user who owns/created the advertisement.
* **`id_property`** (FK -> `Property.id_property`, INT): Unique identifier of the associated property (see `PropertyDetails.md`).
* **`id_superseded_advertisement`** (FK -> `Advertisement.id_advertisement`, UUID, NULLABLE): The version this version replaces (set only on replacement versions created by `UPDATE`; on approval the superseded version becomes `DELETED`).
* **`publish_date`** (DATETIME/TIMESTAMP, NULLABLE): The date when the **first version** of the advertisement became `ACTIVE`. Used to order the feed (latest first). **Preserved across replacements** — a replacement version inherits the original `publish_date`, it is never refreshed.
* **`updated_at`** (DATETIME/TIMESTAMP, NULLABLE): `NULL` at creation; set to now on every update (PENDING/DRAFT in-place edits and creation of replacement versions).
